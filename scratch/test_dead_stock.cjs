...
const DeadStock = require("../server/models/DeadStock");
const Store = require("../backend/models/Store");
const { getDeadStock, getDeadStockSummary } = require("../server/controllers/deadStockController");

async function testEngineAndDatabase() {
  console.log("--- STARTING DEAD STOCK MODULE VERIFICATION ---");
  const mongoUri = process.env.MONGODB_URI || process.env.MONGO_URI;
  console.log("Connecting to MongoDB Atlas...", mongoUri ? "URI FOUND" : "NO URI FOUND");

  // Disable Mongoose command buffering so any connection issue fails fast instead of hanging 10s
  mongoose.set("bufferCommands", false);

  await mongoose.connect(mongoUri, {
    serverSelectionTimeoutMS: 5000,
  });
  console.log("Connected to MongoDB Atlas ✅ Connection State:", mongoose.connection.readyState);
  ...
  console.log("Store document ready ✅");
  ...
  console.log("Dead stock products list snippet:", JSON.stringify(listResData?.data?.map(i => ({ title: i.title, sku: i.sku, daysUnsold: i.daysUnsold, stock: i.stock, cashTiedUp: i.cashTiedUp })), null, 2));
